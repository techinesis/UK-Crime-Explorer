...
lsoa_map = lsoa_map[["LSOA11CD", "LSOA11NM", "LAD11NM", "geometry"]].copy()

# Rename columns
lsoa_map = lsoa_map.rename(
    columns={"LSOA11CD": "lsoa_code", "LSOA11NM": "lsoa_name", "LAD11NM": "borough"}
)
...
lsoa_map["geometry"] = lsoa_map["geometry"].simplify(tolerance=0.0005, preserve_topology=True)

# Save clean boundaries
lsoa_map.to_file("outputs/london_lsoa_boundaries_clean.geojson", driver="GeoJSON")
...
borough_map = borough_map.rename(columns={"GSS_CODE": "borough_code", "NAME": "borough"})

borough_map = borough_map.to_crs(epsg=4326)

borough_map["geometry"] = borough_map["geometry"].simplify(tolerance=0.0005, preserve_topology=True)

borough_map.to_file("outputs/london_borough_boundaries_clean.geojson", driver="GeoJSON")

print("Loading ward boundary data...")
ward_map = gpd.read_file(ward_boundary_path)

ward_map = ward_map[["GSS_CODE", "NAME", "LB_GSS_CD", "BOROUGH", "geometry"]].copy()
...
ward_map["geometry"] = ward_map["geometry"].simplify(tolerance=0.0005, preserve_topology=True)

ward_map.to_file("outputs/london_ward_boundaries_clean.geojson", driver="GeoJSON")
